...
def update_search():
    text.delete(0.0,END)
    search_outcome=data.filtered_data.df
...
def update_sort():
    text.delete(0.0, END)
    sort_outcome = data.sample_data.df
...
b1=Button(myWindow, text='go',font=('Helvetica 10 bold'),width=8, height=1,command=update_search)
b1.place(relx=0.8,rely=0.2)
Label(myWindow, text="sort method",font=('Arial 8 bold'),width=15,height=1).place(relx=0.1,rely=0.4)
Label(myWindow, text="number",font=('Arial 8 bold'),width=15,height=1).place(relx=0.5,rely=0.4)
...
b2=Button(myWindow, text='go', font=('Helvetica 10 bold'),width=8, height=1,command=update_sort)
