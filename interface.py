...
def update_sort():
    ...
    text.delete(0.0, END)
    text.configure(state='disabled')
    if str(e1.get()).isdigit()==False:
        text.configure(state='normal')
        text.insert('end', 'Your input is illegal')
        text.configure(state='disabled')
    else:
        number_of_entry_to_show, column_index, ascending = interface_controller.get_content_sort([combobox2, e1, combobox3])
...
#sort part
Label(myWindow, text="sort", font=('Arial 8 bold'), width=10, height=2).place(rely=0.22, relx=0.01)
Label(myWindow, text="sort method", font=('Arial 8 bold'), width=15, height=1).place(relx=0.1, rely=0.3)
Label(myWindow, text="sorting column", font=('Arial 8 bold'), width=15, height=1).place(relx=0.3, rely=0.3)
Label(myWindow, text="number of entry to show", font=('Arial 8 bold'), width=20, height=1).place(relx=0.5, rely=0.3)
...
combobox3 = ttk.Combobox(master=myWindow, state='readonly', textvariable=sort_column, values=sort_columns,width=15)
